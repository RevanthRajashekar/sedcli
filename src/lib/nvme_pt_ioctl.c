				    ...
				    uint64_t offset, uint64_t size)
{
	...
	if (size > len || offset > len - size) {
		SEDCLI_DEBUG_PARAM("The data doesn't fit in the table "\
				   "(%lu v/s %lu)\n", offset + size, len);
		return -ENOSPC;
	}
...
static int opal_write_datastr(int fd, struct opal_device *dev, const uint8_t *data,
			      uint64_t offset, uint64_t size)
{
...
static struct opal_req_item opal_genric_read_table_cmd[] = {
	{ .type = OPAL_U8, .len = 1, .val = { .byte = OPAL_STARTLIST } },
	{ .type = OPAL_U8, .len = 1, .val = { .byte = OPAL_STARTNAME } },
				   ...
				   uint64_t offset, uint64_t size)
{
	...
	while (index < end_row) {
		opal_genric_read_table_cmd[3].val.uint = index + offset;

		len = MIN(OPAL_MAX_READ_TABLE, (end_row - index));
		opal_genric_read_table_cmd[7].val.uint = index + offset + len;

		prepare_req_buf(dev, opal_genric_read_table_cmd, ARRAY_SIZE(opal_genric_read_table_cmd),
				opal_uid[table], opal_method[OPAL_GET_METHOD_UID]);
